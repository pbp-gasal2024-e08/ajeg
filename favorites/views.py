...
def favorite_product(request):
    ...
    favorite_list, created = FavoriteProductList.objects.get_or_create(
        user=request.user
    )
    favorite_item = FavoriteProduct.objects.filter(
        favorite_list=favorite_list, product_id=product_id
    )
...
def view_favorites(request):
    favorite_product_list, created = FavoriteProductList.objects.get_or_create(
        user=request.user
    )
    favorite_store_list, created = FavoriteStoreList.objects.get_or_create(
        user=request.user
    )
