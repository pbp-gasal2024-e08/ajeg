        ...
        if confirm == "y":
            ...
            for user_data in users:
                ...
                ajeg_user = AjegUser.objects.create(
                    ajeg_user=user, user_type=user_data["user_type"]
                )
                if not first_user:
                    first_user = user
                self.stdout.write(
                    self.style.SUCCESS(
                        f'[{ajeg_user.user_type}] User {user.username} created. with password {user_data["password"]}'
                    )
                )
            ...
            for product_id in product_ids[
                :2
            ]:  # Assuming you want the first two products in the wishlist
