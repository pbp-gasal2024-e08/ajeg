    ...
    if request.method == "POST":  # Pastikan ini adalah permintaan POST
        ...
        try:
            ...
            if not voucher.is_claimed and voucher.expiry_date > timezone.now():
                ...
                return JsonResponse(
                    {
                        "message": f"You successfully applied Code: {voucher.code} to your cart!",
                        "status": "success",
                    }
                )
            return JsonResponse(
                {
                    "message": "Voucher already claimed or expired.",
                    "status": "error",
                }
            )
    ...
    if request.method == "POST":
        try:
            ...
            voucher = Voucher.objects.get(
                code=code, is_claimed=True, expiry_date__gt=timezone.now()
            )
            ...
            return JsonResponse(
                {
                    "message": "Voucher not found or not claimed.",
                    "status": "error",
                }
            )
            ...
            return JsonResponse(
                {
                    "message": "An error occurred while adding voucher to cart.",
                    "status": "error",
                }
            )
    else:
